...
def plot_pca_or_tsne_layerwise(
    ...
    # KEPT FROM FIXED: Convert to numpy arrays with explicit checks
    ...
    # KEPT FROM FIXED: Fixed subplot calculation logic with explicit checks
    if (n_layers - 1) % 6 == 0:
        n_rows = (n_layers - 1) // 6
    else:
        n_rows = (n_layers - 1) // 6 + 1

    # KEPT FROM FIXED: Ensure minimum 1 row
    if n_rows == 0:
        n_rows = 1

    fig, axes = plt.subplots(n_rows, 6, figsize=(24, 13))

    # KEPT FROM FIXED: Handle single row case
    if n_rows == 1:
    ...
    for layer_idx in range(1, n_layers):
        ...
        # KEPT FROM FIXED: Enhanced standardization with explicit variance checking
        if standardize:
            data_mean = np.mean(states_data, axis=0)
            data_std = np.std(states_data, axis=0)
            # KEPT FROM FIXED: Explicit check for zero variance features
        ...
        # KEPT FROM FIXED: Enhanced numerical stability checks for PCA
        if mode == "pca":
            ...
            if np.sum(non_constant_features) > 0:
                ...
                # KEPT FROM FIXED: Explicit check for valid n_components
            ...
            # KEPT FROM FIXED: Add explicit check for valid data
            if (
        ...
        if X_proj.shape[1] > max(component_0, component_1):
            ...
            # KEPT FROM FIXED: Handle case where projection has insufficient components
            ax.text(
                0.5,
                0.5,
                f"Insufficient data\nfor Layer {layer_idx}",
