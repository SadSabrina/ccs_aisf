...
class CCS(object):
    ...
    def __init__(
        ...
        # Changed: Convert data to GPU tensors immediately and keep on GPU
        self.x0 = self.normalize_gpu(
            torch.tensor(x0, dtype=torch.float32, device=self.device)
        )
        self.x1 = self.normalize_gpu(
            torch.tensor(x1, dtype=torch.float32, device=self.device)
        )

        # Changed: Keep labels on GPU if provided
        if y_train is not None:
            self.y_train = torch.tensor(
                y_train, dtype=torch.float32, device=self.device
            )
        else:
            self.y_train = None
    ...
    def initialize_probe(self):
        # Changed: Always initialize on GPU
        if self.linear:
    ...
    def normalize_gpu(self, x):
        # Changed: GPU-only normalization
    ...
    def normalize(self, x):
        # Keep original function for compatibility, but update for GPU
    ...
    def get_tensor_data(self):
        # Changed: Data already on GPU, just return
    ...
    def get_loss(self, p0, p1, y_true=None):
        ...
        if self.lambda_classification != 0 and y_true is not None:
            ...
            # Changed: Explicit shape checking instead of implicit conversion
    ...
    def predict(self, x0_test, x1_test, predict_normalize=False):
        # Changed: Keep everything on GPU, only move to CPU when explicitly needed by caller
    ...
    def get_silhouette(self, x0_test, x1_test):
        predictions, _ = self.predict(x0_test, x1_test)
        # Changed: Explicit check for unique predictions
    ...
    def get_contrastive_probas(self, xA0_test, xA1_test, x_notA0_test, x_notA1_test):
        # Changed: Keep operations on GPU until final conversion
        xA0_test = torch.tensor(
            self.normalize(xA0_test), dtype=torch.float32, device=self.device
        )
        xA1_test = torch.tensor(
            self.normalize(xA1_test), dtype=torch.float32, device=self.device
        )

        x_notA0_test = torch.tensor(
            self.normalize(x_notA0_test), dtype=torch.float32, device=self.device
        )
        x_notA1_test = torch.tensor(
            self.normalize(x_notA1_test), dtype=torch.float32, device=self.device
        )

        with torch.no_grad():
            pA0, pA1 = (
                self.best_probe(xA0_test).detach().cpu().numpy(),
                self.best_probe(xA1_test).detach().cpu().numpy(),
            )
            p_notA0, p_notA1 = (
                self.best_probe(x_notA0_test).detach().cpu().numpy(),
                self.best_probe(x_notA1_test).detach().cpu().numpy(),
            )

        return pA0, pA1, p_notA0, p_notA1

    def get_agreement(self, xA0_test, xA1_test, x_notA0_test, x_notA1_test):
        pA0, pA1, p_notA0, p_notA1 = self.get_contrastive_probas(
            xA0_test, xA1_test, x_notA0_test, x_notA1_test
        )
    ...
    def get_ideal_distance(self, xA0_test, xA1_test, x_notA0_test, x_notA1_test):
        ...
        ideal_dist = 0.5 * (
            np.sqrt(
                (pA1 - np.ones(pA1.shape)) ** 2
                + (pA0 - np.zeros(pA0.shape)) ** 2
                + (p_notA1 - np.zeros(p_notA1.shape)) ** 2
                + (p_notA0 - np.ones(p_notA0.shape)) ** 2
            )
        )
        return ideal_dist
    ...
    def train(self):
        ...
        # Changed: Keep y_train on GPU if it exists
        y_tensor = None
        if self.lambda_classification != 0 and self.y_train is not None:
            y_tensor = self.y_train.clone()
            if y_tensor.ndim == 1:
        ...
        for epoch in range(self.nepochs):
            # Changed: Generate permutation on GPU
            ...
            for j in range(nbatches):
                x0_batch = (
                    x0[j * batch_size : (j + 1) * batch_size]
                    .to(self.device)
                    .to(torch.float32)
                )
                x1_batch = (
                    x1[j * batch_size : (j + 1) * batch_size]
                    .to(self.device)
                    .to(torch.float32)
                )
                y_batch = (
                    y_tensor[j * batch_size : (j + 1) * batch_size]
                    if y_tensor is not None
                    else None
                )
    ...
    def get_weights(self):
        ...
        Returns:
            ...
            bias: float
        """
        # Changed: Explicit check instead of implicit assumption
        if not self.linear:
            raise ValueError("Weights can only be extracted from linear probes.")

        # Since we checked self.linear, self.best_probe is guaranteed to be nn.Sequential
        linear_layer = self.best_probe[0]  # type: ignore
        weight = (
            linear_layer.weight.detach().cpu().numpy().squeeze()  # type: ignore
        )  # shape: (1, d) -> (d,)
        bias = linear_layer.bias.detach().cpu().numpy().item()  # type: ignore

        return weight, bias

    def repeated_train(self):
        # Changed: Use explicit float instead of np.inf
        best_loss = float("inf")
...
def train_ccs_on_hidden_states(
    X_pos,
    X_neg,
    y_vec,
    train_idx,
    ...
    for layer_idx in range(n_layers):
        # X positive (yes)
        X_pos_train_layer = X_pos[train_idx, layer_idx, :].astype(
            np.float32
        )  # (train_samples, hidden_dim)
        ...
        # Changed: Explicit check instead of implicit condition
        if normalizing is None or normalizing == "raw":
            pass
        ...
        ccs_ideal_dist = ccs.get_ideal_distance(
            A0_test, A1_test, notA0_test, notA1_test
        )
...
def train_half_ccs_on_hidden_states(
    X_pos,
    X_neg,
    y_vec,
    random_state=71,
    ...
    train_idx, test_idx = train_test_split(
        all_idx, test_size=0.15, random_state=71, shuffle=True
    )
    ...
    for layer_idx in range(n_layers):
        # first cluster pos
        X_pos_first_cluster_train_layer = first_pos_cluster[
            train_idx, layer_idx, :
        ].astype(np.float32)  # (train_samples, hidden_dim)
        X_pos_first_cluster_test_layer = first_pos_cluster[
            test_idx, layer_idx, :
        ].astype(np.float32)

        # second cluster pos
        X_pos_second_cluster_train_layer = second_pos_cluster[
            train_idx, layer_idx, :
        ].astype(np.float32)  # (train_samples, hidden_dim)
        X_pos_second_cluster_test_layer = second_pos_cluster[
            test_idx, layer_idx, :
        ].astype(np.float32)

        # first cluster neg
        X_neg_first_cluster_train_layer = first_neg_cluster[
            train_idx, layer_idx, :
        ].astype(np.float32)  # (train_samples, hidden_dim)
        X_neg_first_cluster_test_layer = first_neg_cluster[
            test_idx, layer_idx, :
        ].astype(np.float32)

        # second cluster neg
        X_neg_second_cluster_train_layer = second_neg_cluster[
            train_idx, layer_idx, :
        ].astype(np.float32)  # (train_samples, hidden_dim)
        X_neg_second_cluster_test_layer = second_neg_cluster[
            test_idx, layer_idx, :
        ].astype(np.float32)
        ...
        ccs_first = CCS(
            X_neg_first_cluster_train_layer,
            X_pos_first_cluster_train_layer,
            y_train_first,
            var_normalize=False,
        ...
        ccs_second = CCS(
            X_neg_second_cluster_train_layer,
            X_pos_second_cluster_train_layer,
            y_train_second,
            var_normalize=False,
            weight_decay=weight_decay,
            lambda_classification=lambda_classification,
        ...
        # Оценка
        predictions_first, conf_first = ccs_first.predict(
            X_neg_first_cluster_test_layer, X_pos_first_cluster_test_layer
        )
        predictions_second, conf_second = ccs_first.predict(
            X_neg_first_cluster_test_layer, X_pos_first_cluster_test_layer
        )

        # Changed: Explicit checks instead of implicit conditions
        unique_predictions_first = np.unique(predictions_first)
        if len(unique_predictions_first) == 1:
            s_score_f = 0
        else:
            s_score_f = ccs_first.get_silhouette(
                X_neg_first_cluster_test_layer, X_pos_first_cluster_test_layer
            )

        unique_predictions_second = np.unique(predictions_second)
        if len(unique_predictions_second) == 1:
            s_score_s = 0
        else:
            s_score_s = ccs_second.get_silhouette(
                X_neg_second_cluster_test_layer, X_pos_second_cluster_test_layer
            )
...
def train_lr_on_hidden_states(
    X_pos, X_neg, y_vec, train_idx, test_idx, random_state=71
):
    """
    ...
    for layer_idx in range(n_layers):
        ...
        # Save results
        results[layer_idx] = {"Accuracy": acc, "Silhouette": s_score}
