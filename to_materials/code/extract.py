...
def extract_representation(
    ...
    # Proper tokenization with max_length to prevent overflow
    inputs = tokenizer(
    ...
    # Get attention mask for proper token identification
    ...
    # Enhanced extraction logic with proper handling for all cases
    if get_all_hs:
        reps = []
        for layer_idx, layer_h in enumerate(hs_tuple):
            #  Convert to float32 immediately to avoid precision issues
            ...
            if strategy == "first-token":
                ...
                #  Use attention mask for proper mean pooling (exclude padding)
                if attention_mask is not None:
            ...
            # Ensure float32 output and handle any remaining numerical issues
            extracted = extracted.astype(np.float32)

            # Replace any inf/nan values with zeros
        ...
        # Final check for extreme values and clip if necessary
...
def vectorize_df(
    ...
    # Stack with proper error handling
    try:
        result = np.stack(embeddings).astype(np.float32)
    except ValueError as e:
        print(f"Error stacking embeddings: {e}")
        print("This might be due to inconsistent shapes. Attempting to fix...")

        #  Handle inconsistent shapes by finding common shape
        max_shape = None
        for emb in embeddings:
            if max_shape is None:
                ...
                max_shape = tuple(
                    max(max_shape[i], emb.shape[i]) for i in range(len(max_shape))
                )

        # Pad all embeddings to max_shape
        padded_embeddings = []
        for emb in embeddings:
            if emb.shape != max_shape:
                if len(emb.shape) == 1:
                    ...
                    padded = emb  # Keep as is if shape is too complex
                padded_embeddings.append(padded)
            else:
                padded_embeddings.append(emb)

        result = np.stack(padded_embeddings).astype(np.float32)
